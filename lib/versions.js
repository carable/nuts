            ...
            return Q.fcall(function () {
                return _.chain(versions)
                    .filter(function (version) {
                        // Check channel
                        if (opts.channel != '*' && version.channel != opts.channel && version.channel != 'stable') return false;
                        ...
                        var tag = semver.valid(opts.tag);
                        if (tag) {
                            return semver.gt(version.tag, tag);
                        }
                        var range = semver.validRange(opts.tag);
                        if (range) {
                            return semver.satisfies(version.tag, range);
                        }
                        return false;
                ...
                if (!v || v.length < 1 || opts.tag === 'latest') {
                    return v;
                }
                var tag = semver.valid(opts.tag);
                if (tag) {
                    ...
                    return _.chain(versions)
                        .filter(function (version) {
                            ...
                            var tag = semver.valid(v[0].tag);
                            if (tag) {
                                return semver.gte(version.tag, tag);
                            }
                            return false;
